package worldgeneratorextension.scatteredbuilding.structure;
...
import worldgeneratorextension.global.task.TileSyncTask;
import worldgeneratorextension.scatteredbuilding.structure.piece.StructurePiece;
...
public class StructureBuilder {
    ...
    public void setTile(BlockVector3 pos, String id, CompoundTag data) {
        ...
        Server.getInstance().getScheduler().scheduleTask(new TileSyncTask(id, chunk, nbt));
    }
    ...
    public void setBlockDownward(BlockVector3 pos, int id, int meta) {
        BlockVector3 vec = this.translate(pos);
        int y = vec.y;
        while (!Block.isBlockSolidById(this.level.getBlockIdAt(vec.x, y, vec.z)) && y > 1) {
