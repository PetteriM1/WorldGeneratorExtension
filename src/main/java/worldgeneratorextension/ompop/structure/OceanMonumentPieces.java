package worldgeneratorextension.ompop.structure;
...
import worldgeneratorextension.global.util.NukkitCollections;
...
public class OceanMonumentPieces {
    ...
    public abstract static class OceanMonumentPiece extends StructurePiece {
        ...
        protected OceanMonumentPiece(int genDepth, BlockFace orientation, RoomDefinition roomDefinition, int x, int y, int z) {
            ...
            if (orientation == BlockFace.NORTH || orientation == BlockFace.SOUTH) {
                this.boundingBox = new BoundingBox(0, 0, 0, (x << 3) - 1, (y << 2) - 1, (z << 3) - 1);
            } else {
                this.boundingBox = new BoundingBox(0, 0, 0, (z << 3) - 1, (y << 2) - 1, (x << 3) - 1);
            }

            switch (orientation) {
                case NORTH:
                    this.boundingBox.move(indexX << 3, indexY << 2, (-(indexZ + z) << 3) + 1);
                    break;
                case SOUTH:
                    this.boundingBox.move(indexX << 3, indexY << 2, indexZ << 3);
                    break;
                case WEST:
                    this.boundingBox.move((-(indexZ + z) << 3) + 1, indexY << 2, indexX << 3);
                    break;
                default:
                    this.boundingBox.move(indexZ << 3, indexY << 2, indexX << 3);
                    break;
            }
        }
    ...
    public static class MonumentBuilding extends OceanMonumentPiece {
        ...
        public boolean postProcess(ChunkManager level, NukkitRandom random, BoundingBox boundingBox, int chunkX, int chunkZ) {
            ...
            for (int i = 0; i < 5; ++i) {
                this.generateWaterBox(level, boundingBox, -1 - i, i << 1, -1 - i, -1 - i, 23, 58 + i);
                this.generateWaterBox(level, boundingBox, 58 + i, i << 1, -1 - i, 58 + i, 23, 58 + i);
                this.generateWaterBox(level, boundingBox, -i, i << 1, -1 - i, 57 + i, 23, -1 - i);
                this.generateWaterBox(level, boundingBox, -i, i << 1, 58 + i, 57 + i, 23, 58 + i);
            }
        ...
        private void generateEntranceArchs(ChunkManager level, NukkitRandom random, BoundingBox boundingBox) {
            if (this.chunkIntersects(boundingBox, 22, 5, 35, 17)) {
                ...
                for (int z = 0; z < 4; ++z) {
                    this.generateBox(level, boundingBox, 24, 2, 5 + (z << 2), 24, 4, 5 + (z << 2), BASE_LIGHT, BASE_LIGHT, false);
                    this.generateBox(level, boundingBox, 22, 4, 5 + (z << 2), 23, 4, 5 + (z << 2), BASE_LIGHT, BASE_LIGHT, false);
                    this.placeBlock(level, BASE_LIGHT, 25, 5, 5 + (z << 2), boundingBox);
                    this.placeBlock(level, BASE_LIGHT, 26, 6, 5 + (z << 2), boundingBox);
                    this.placeBlock(level, LAMP_BLOCK, 26, 5, 5 + (z << 2), boundingBox);
                    this.generateBox(level, boundingBox, 33, 2, 5 + (z << 2), 33, 4, 5 + (z << 2), BASE_LIGHT, BASE_LIGHT, false);
                    this.generateBox(level, boundingBox, 34, 4, 5 + (z << 2), 35, 4, 5 + (z << 2), BASE_LIGHT, BASE_LIGHT, false);
                    this.placeBlock(level, BASE_LIGHT, 32, 5, 5 + (z << 2), boundingBox);
                    this.placeBlock(level, BASE_LIGHT, 31, 6, 5 + (z << 2), boundingBox);
                    this.placeBlock(level, LAMP_BLOCK, 31, 5, 5 + (z << 2), boundingBox);
                    this.generateBox(level, boundingBox, 27, 6, 5 + (z << 2), 30, 6, 5 + (z << 2), BASE_GRAY, BASE_GRAY, false);
                }
            }
        }
        ...
        private void generateEntranceWall(ChunkManager level, NukkitRandom random, BoundingBox boundingBox) {
            if (this.chunkIntersects(boundingBox, 15, 20, 42, 21)) {
                ...
                for (int x = 0; x < 3; ++x) {
                    this.placeBlock(level, BASE_BLACK, 22 - (x << 1), 8, 21, boundingBox);
                    this.placeBlock(level, BASE_BLACK, 22 - (x << 1), 9, 21, boundingBox);
                    this.placeBlock(level, BASE_BLACK, 35 + (x << 1), 8, 21, boundingBox);
                    this.placeBlock(level, BASE_BLACK, 35 + (x << 1), 9, 21, boundingBox);
                }
